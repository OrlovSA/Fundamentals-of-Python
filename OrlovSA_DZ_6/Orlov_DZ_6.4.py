# 4) Реализуйте базовый класс Car. У данного класса должны быть следующие атрибуты: speed,
# color, name, i s_police ( булево). А также методы: go, stop, t urn(direction), которые должны
# сообщать, что машина поехала, остановилась, повернула (куда). Опишите несколько
# дочерних классов: TownCar, SportCar, WorkCar, PoliceCar. Добавьте в базовый класс метод
# show_speed, который должен показывать текущую скорость автомобиля. Для классов
# TownCar и WorkCar переопределите метод show_speed. При значении скорости свыше 60
# ( TownCar ) и 40 ( WorkCar ) должно выводиться сообщение о превышении скорости.
# Создайте экземпляры классов, передайте значения атрибутов. Выполните доступ к
# атрибутам, выведите результат. Выполните вызов методов и также покажите результат.


class Car:
    def __init__(self, speed=0, color='Blue', name='Honda', is_police=True):
        self.speed = speed
        self.color = color
        self.name = name
        self.s_police = is_police
        print('Машина создана')



    def go(self):
        print(f"Машина поехала со скоростью {self.speed}")
        self.show_speed()

    def stop(self):
        print("Машина остановилась")


    def turn(self, direction="прямо"):
        return 'машина движется в направлении ' + direction


    def show_speed(self):
        if self.speed >= 60 and self.s_police != True:
            return print('Привышение скорости')


class TownCar(Car):
    def __init__(self, speed=0, color='Blue', name='Honda', is_police=False):
        super().__init__(speed, color, name, is_police)
        self.speed = 70


class SportCar(Car):
    def __init__(self, speed=0, color='Blue', name='Honda', is_police=False):
        super().__init__(speed, color, name, is_police)



class WorkCar(Car):
    def __init__(self, speed=0, color='Blue', name='Honda', is_police=False):
        super().__init__(speed, color, name, is_police)
        self.speed = 70


class PoliceCar(Car):
    def __init__(self, speed=0, color='Blue', name='Honda', is_police=True):
        super().__init__(speed, color, name, is_police)
        self.speed = 90


result_1 = PoliceCar()
result_1.go()
result_2 = WorkCar()
result_2.go()